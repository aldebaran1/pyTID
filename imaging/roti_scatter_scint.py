...
#    data = gpsdata['roti'][:]
    ...
    if platform.system() == 'Linux':
        ...
        odir += dirdatetime + '_' + str(int(altkm)) + '_' + str(average) + '_' + cmap + '_' + str(clim[1]).replace('.', '') + '_' + str(abs(lonlim[0])) + '_' + str(abs(lonlim[1]))
        if P.scint is not None:
            odir += '_scint'
        odir += '/'
...
#            rotia = np.nanmedian(gpsdata['roti'][i-average:i+1, :, :], axis=0) * 60
            ...
            if i > average and i < dt.size:
                ...
                rotia = np.nanmedian(gpsdata['roti'][i-average:i], axis=0) * 60
            rotia = rotia[idf]
            glon = glon[idf]
            glat = glat[idf]
            # Z-axis
            idf1 = (rotia < 0.3 * clim[1])
            idf2 = (rotia >= 0.3 * clim[1]) & (rotia < 0.6 * clim[1])
            idf3 = (rotia > 0.6 * clim[1]) #& (rotia <= clim[1])

            imax = ax.scatter(glon[idf1], glat[idf1], c=rotia[idf1], 
                               s = scatter_size, alpha=0.9, zorder=1,
                               cmap=cmap, 
                               vmin = clim[0], vmax = clim[1],
                               transform=ccrs.PlateCarree())
            imax = ax.scatter(glon[idf2], glat[idf2], c=rotia[idf2], 
                               s = scatter_size, alpha=0.9, zorder=2,
                               cmap=cmap, 
                               vmin = clim[0], vmax = clim[1],
                               transform=ccrs.PlateCarree())
            imax = ax.scatter(glon[idf3], glat[idf3], c=rotia[idf3], 
