...
def do_one(fnc, i, f, window_size, use='G'):
    global leap_seconds, fsp3, polynom_list, E0, el_mask, t, ts, compute_times, Hipp
    # try:
    L = len(fnc)
    for l in range(1,L):
        if l == 1:
    ...
    N0 = int((60/tsps)*5) # 5 min average for a baseline bandpass filtering
    ...
    DCB = pyGnss.getDCBfromSTEC(STEC, AER, el_mask=el_mask, H=Hipp, ROTI=ROTI, ROTIc=0.5)
    STECcorr = STEC - DCB
    if os.path.split(fsp3[0])[1][:4] == 'brdc':
        DTEC = pyGnss.getDTEC2(STECcorr, eps=eps, tsps=tsps, polynom_list=polynom_list)
        DTEC0 = pyGnss.getDTECsg_from_VTEC(STECcorr, N=N0, order=1)
        DTECsg = pyGnss.getDTECsg_from_VTEC(STECcorr, N=N, order=1)
    else:
        F = pyGnss.getMappingFunction(AER[:,:,1], Hipp)
        VTEC = STECcorr * F
        DTEC = pyGnss.getDTEC2(VTEC, eps=eps, tsps=tsps, polynom_list=polynom_list)
        DTE0 = pyGnss.getDTECsg_from_VTEC(VTEC, N=N0, order=1)
        DTECsg = pyGnss.getDTECsg_from_VTEC(VTEC, N=N, order=1)

    SNR = pyGnss.getCNR(D, fsp3=None, el_mask=el_mask, H=Hipp)
    SNR[idel] = np.nan
    try:
        rxmodel = D.rxmodel
    except:
    ...
    # ax.plot(dt, STECcorr)
    # ax2.plot(t, STECcorr[idt_reverse])
    ...
    if dt[idt_reverse].size != t.size:    
        for j, isv in enumerate(np.where(isv_reverse)[0]):
            with h5py.File(f,'r+') as ds:
                ...
                ds['res0'][idt_original, isv, i] = DTEC0[idt_reverse, j]
        ...
        with h5py.File(f,'r+') as ds:
            ...
            ds['res0'][:, isv_reverse, i] = DTEC0[idt_reverse]
