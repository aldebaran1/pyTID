...
#    p.add_argument('day', type=str)
    ...
    p.add_argument('-o', '--ofn', help='Output filename with or withou root folder. Default destiantion is {}'.format(SAVEFOLDER), default=None)
    ...
    if P.ofn is None:
        ...
        if os.path.isfile(P.ofn): 
            ...
            assert os.file.splitext(P.ofn)[1] in ('.h5', '.hdf5')
            savefn = os.path.join(P.ofn, os.path.split(rxlist)[1] + '_' + str(year) + '_' + str(int(ipp_alt)) +'km.h5')
        else:
            assert os.file.splitext(P.ofn)[1] in ('.h5', '.hdf5')
        ...
        try:
            ...
            for isv, sv in enumerate(svlist):
                ...
                try:
                    ...
                    stec += sb
                    F = pyGnss.getMappingFunction(elv, ipp_alt)
                    tec = stec * F
    ...
    if os.path.exists(savefn):
        c = 1
        while not os.path.exists(savefn):
            savefn = _sfnadd(savefn, c)
